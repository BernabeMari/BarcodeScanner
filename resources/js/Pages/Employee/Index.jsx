      ...
      style={{ backgroundImage: "linear-gradient(rgba(0,0,0,0.6), rgba(0,0,0,0.6)), url('/images/TCU.jpg')"  }}>
                ...
                <EmployeeLayout>
                    ...
                    <div className="flex flex-row justify-center">
                        <button onClick={() => {setShowSingleRequest(true); setShowMultipleRequest(false)}} type="button" className="bg-blue-400 rounded-full m-10 p-3">Single Requests</button>
                        <button onClick={() => {setShowMultipleRequest(true); setShowSingleRequest(false)}} type="button" className="bg-yellow-400 rounded-full m-10 p-3">Multiple Requests</button>
                    </div>
                    {/* Multiple Requests */}
                    {showMultipleRequest && (
                        <div>
                        ...
                        <div className="flex flex-row justify-center">
                            ...
                            onChange={(e) => {
                                ...
                                setRequestData('request_type', 'multiple')
                            }}
                            />
                        ))}
                        </div>
                        ...
                        <div className="flex-col flex bg-red-300 m-10 rounded-3xl">
                            ...
                            <input type="number" value={request_quantity} placeholder="Enter Qty (per pack)" className="rounded-full m-4"
                            onChange={(e) => {
                        ...
                        <button type="button" onClick={(e) => {setRequestData('message', [...requestData.message, ""]);setRequestData('request_quantity', [...requestData.request_quantity, ""])}} className="bg-green-300 m-4 rounded-3xl hover:bg-green-800">+</button>
                        ...
                        <div className="flex flex-row justify-center">
                            <div className="flex-col flex bg-red-300 m-10 rounded-3xl">
                                ...
                                onChange={(e) => {
                                    ...
                                    newRequest[index] = e.target.value
                                    setRequestData('message', newRequest)
                                    setRequestData('request_type', 'single')
                                }}
                                />
                            ))}
                            </div>
                            ...
                            <div className="flex-col flex bg-red-300 m-10 rounded-3xl">
                                {requestData.request_quantity.map((request_quantity, index) => (
                                <input type="number" value={request_quantity} placeholder="Enter Qty (per piece)" className="rounded-full m-4"
                                onChange={(e) => {
                                    const newRequestQty = [...requestData.request_quantity]
                                    newRequestQty[index] = e.target.value
                                    setRequestData('request_quantity', newRequestQty)
